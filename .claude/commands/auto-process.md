...
5. **Mark for review & stop**
   ...
   - Always present/print the complete final dialogue JSON directly in the chat response (not just the Linear comment), and share the output file itself, so the user can review it without needing to open Linear.
...
6. **User validation, correction & push (manual, on confirmation only)**
   ...
   - After applying any correction, always present/print the complete corrected dialogue JSON directly in the chat response (not just a description of the change) so the user can review the exact result.
