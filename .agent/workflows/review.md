...
4. Perform a comprehensive review of the dialogue based strictly on the instructions in `prompts/review-dialogue-guidelines.md`. Loop through each segment of the dialogue and perform the review. This includes:
5. **Notify User**:
...
6. Ask user about all issues if it need to be corrected. 
- if user says yes, use /correction workflow and do the correction for that issue. 
- if user says skip something, then apply only those which user agrees.